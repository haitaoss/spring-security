...
public class OverrideDefaultConfig {
     ...
     * 我们可以通过这个拦截
     *
     * 这个 ObjectPostProcessorConfiguration 会注册用以 依赖注入、初始化 bean 的 ObjectPostProcessor
     * @return
     */
    @Bean
    @Primary
    public ObjectPostProcessor<Object> myObjectPostProcessor(List<ObjectPostProcessor<Object>> delegate) {
        return new ObjectPostProcessor<Object>() {
            ...
            public <O> O postProcess(O object) {
                ...
                for (ObjectPostProcessor<Object> objectObjectPostProcessor : delegate) {
                    object = objectObjectPostProcessor.postProcess(object);
                }
                return object;
            }
        };
